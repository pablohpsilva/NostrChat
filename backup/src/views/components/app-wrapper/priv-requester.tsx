  ...
  return new Promise((resolve, reject) => {
    ...
    const handleResolve = (ev: CustomEvent) => {
      window.removeEventListener(
        "resolve-priv",
        handleResolve as EventListener
      );
...
const PrivRequiredDialog = (props: {
  data: any;
  ...
  const [userKey, setUserKey] = useState(
    tempPriv ? nip19.nsecEncode(tempPriv) : ""
  );
  ...
  if (isEvent) {
    subTitle = t(
      "Please provide your private key in nsec format to sign this event:"
    );
  } else if (isObject) {
    subTitle = t(
      "Please provide your private key in nsec format to encrypt this message:"
    );
  } else {
    subTitle = t(
      "Please provide your private key in nsec format for decryption"
    );
  }
  ...
  return (
    <>
        ...
        <TextField
          ...
          helperText={
            isInvalid ? (
              ...
              >
                {t(
                  "This will stay in memory and be remembered until you refresh page/app."
                )}{" "}
              </Box>
            )
          }
      ...
      >
        <Button onClick={handleClose} sx={{ mr: "6px" }}>
          {t("Skip")}
        </Button>
...
const PrivRequester = () => {
  ...
  const handleRequest = (ev: CustomEvent) => {
    setTimeout(() => {
      // use a timer to solve modal rendering conflicts.
      showModal({
        body: (
          <PrivRequiredDialog
            data={ev.detail.data}
            onSuccess={(key) => {
              window.dispatchEvent(
                new CustomEvent("resolve-priv", { detail: { key } })
              );
              showModal(null);
            }}
            onHide={rejected}
